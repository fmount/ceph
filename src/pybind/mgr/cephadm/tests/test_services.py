...
from ceph.deployment.service_spec import IscsiServiceSpec, MonitoringSpec, AlertManagerSpec, \
    ServiceSpec, RGWSpec, NFSServiceSpec, GrafanaSpec, SNMPGatewaySpec, IngressSpec, PlacementSpec
...
class TestIngressServiceNFS:
    ...
    def test_ingress_config(self, _run_cephadm, cephadm_module: CephadmOrchestrator):
        ...
        with with_host(cephadm_module, 'test'):
            ...
            # the ingress backend
            nfs_spec = NFSServiceSpec(service_type='nfs', service_id="foo", placement=PlacementSpec(count=1))
            ...
            cephadm_module.set_store(mode, 'true')
            with with_service(cephadm_module, nfs_spec) as _, with_service(cephadm_module, ispec) as _:
                # generate the keepalived conf based on the specified spec
                keepalived_generated_conf = cephadm_module.cephadm_services['ingress'].keepalived_generate_config(
                    CephadmDaemonDeploySpec(host='test', daemon_id='ingress', service_name=ispec.service_name()))
                ...
                with open('/tmp/keepalived_generated_conf', 'w') as f:
                    f.write(str(keepalived_generated_conf[0]))

                with open('/tmp/keepalived_expected_conf', 'w') as f:
                    f.write(str(keepalived_expected_conf))
